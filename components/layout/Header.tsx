...
const UGG_BRANDS = ["UGG", "EMU Australia", "Mou", "Bearpaw", "Koolaburra"];
...
export default function Header() {
	...
	const [expandedBags, setExpandedBags] = useState(false);
  const [expandedUgg, setExpandedUgg] = useState(false);
  const [expandedBagBrand, setExpandedBagBrand] = useState<string | null>(null);
	...
	const closeAll = () => {
		...
		setExpandedBags(false);
    setExpandedUgg(false);
    setExpandedBagBrand(null);
	};
      ...
      <header className="fixed top-0 w-full z-50 bg-white border-b border-gray-100">
        ...
        <div className="hidden md:flex items-center justify-between h-14 md:h-16 px-3 md:px-16 max-w-[1440px] mx-auto gap-2 md:gap-4">
          ...
          <nav className="flex items-center gap-6 ml-6">
            ...
            <Link
              href="/catalog?type=Bags"
              className="text-xs font-bold uppercase text-gray-500 hover:text-black transition tracking-wider">
              Сумки
            </Link>
            <Link
              href="/catalog?type=Uggs"
              className="text-xs font-bold uppercase text-gray-500 hover:text-black transition tracking-wider">
              Угги
            </Link>
            {user?.role === "admin" && (
      ...
      <div
        ...
        <aside
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-1">
            ...
            <div>
              <button
                onClick={() => setExpandedBags(!expandedBags)}
                className="w-full flex items-center justify-between text-sm font-bold uppercase text-gray-600 hover:text-black py-2 border-b border-gray-100 transition">
                Сумки
                <svg
                  className={`w-4 h-4 transition-transform duration-300 ${expandedBags ? "rotate-90" : ""}`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2">
              ...
              <div
                className={`grid transition-all duration-300 ${expandedBags ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                <div className="overflow-hidden">
                  <div className="pl-4 space-y-1 pt-1">
                    {BAG_BRANDS.map((brand) => (
                      <Link
                        key={brand}
                        href={`/catalog?type=Bags&brand=${encodeURIComponent(brand)}`}
                        onClick={closeAll}
                        className="block text-sm font-bold uppercase text-gray-500 hover:text-black py-2 transition">
                        {brand}
                      </Link>
            ...
            <div>
              <button
                onClick={() => setExpandedUgg(!expandedUgg)}
                className="w-full flex items-center justify-between text-sm font-bold uppercase text-gray-600 hover:text-black py-2 border-b border-gray-100 transition">
                Угги
                <svg
                  className={`w-4 h-4 transition-transform duration-300 ${expandedUgg ? "rotate-90" : ""}`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2">
              ...
              <div
                className={`grid transition-all duration-300 ${expandedUgg ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                <div className="overflow-hidden">
                  <div className="pl-4 space-y-1 pt-1">
                    {UGG_BRANDS.map((brand) => (
                      <Link
                        key={brand}
                        href={`/catalog?type=Uggs&brand=${encodeURIComponent(brand)}`}
                        onClick={closeAll}
                        className="block text-sm font-bold uppercase text-gray-500 hover:text-black py-2 transition">
                        {brand}
                      </Link>

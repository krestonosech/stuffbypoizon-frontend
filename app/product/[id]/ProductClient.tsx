...
export default function ProductClient({ id }: { id: string }) {
  ...
  const handlePhoneChange = (value: string) => {
    ...
    setQuickForm((prev) => ({ ...prev, phone: masked }));
  };
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  if (!quickForm.name.trim()) {
                    setQuickError("Введите имя");
                    return;
                  }
                  if (
                    !quickForm.phone.trim() ||
                    quickForm.phone.replace(/\D/g, "").length < 11
                  ) {
                    setQuickError("Введите корректный номер телефона");
                    return;
                  }
                  if (!user && !quickForm.email.trim()) {
                    setQuickError("Введите email");
                    return;
                  }
                  if (
                    !user &&
                    ...
                    setQuickError("Введите корректный email");
                    return;
                  }
                  if (
                    quickForm.messenger === "telegram" &&
                    !quickForm.telegramNick.trim()
                  ) {
                    setQuickError("Введите никнейм в Telegram");
                    return;
                  }
                  if (!agreed) {
                    setQuickError(
                  ...
                  if (!selectedSku) {
                    setQuickError("Выберите размер");
                    return;
                  }
                  setQuickSubmitting(true);
                  setQuickError("");
                  try {
                    const response = await api.post("/orders/quick", {
                      skuId: selectedSku.id,
                      quantity: 1,
                      ...quickForm,
                    });
                    if (response.data.createdAccount) {
                      setCreatedAccount(response.data.createdAccount);
                    }
